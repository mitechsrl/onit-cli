...
exports.directoryConfigsScanner = void 0;
...
/**
  ...
  * @returns
  */
async function directoryConfigsScanner(dir) {
    // load all the directories in this path. these are the configs
    const dirContent = await fs_1.default.promises.readdir(dir);
    ...
    const configs = await Promise.all(dirContent.map(async (d) => {
        d = path_1.default.join(dir, d);
        let config = null;
        if ((await fs_1.default.promises.stat(d)).isDirectory()) {
            try {
...
exports.directoryConfigsScanner = directoryConfigsScanner;
//# sourceMappingURL=directoryConfigsScanner.js.map